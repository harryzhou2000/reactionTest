"""
Test using combined (flow+source) Jacobian for bandpass.
"""
...
def make_compute_chi_split(use_combined):
    def compute_chi_split(self, u, dt, threshold=None, width=None, transition="inv"):
        ...
        if use_combined:
            # Use combined stiffness (flow + source)
...
def run_case(case_name, use_combined):
    AdvReactUni1DEval.compute_chi_split = make_compute_chi_split(use_combined)
...
print("Testing combined (flow+source) Jacobian for bandpass:")
print()

for use_combined in [False, True]:
    label = "combined" if use_combined else "source only"
    errs = {case: run_case(case, use_combined) for case in ['A', 'B', 'C']}
    print(f"{label:<15s}: A={errs['A']:.2e}, B={errs['B']:.2e}, C={errs['C']:.2e}")
