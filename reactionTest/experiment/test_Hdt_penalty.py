"""
Test H*dt penalty as scalable alternative to absolute H_norm > 800.
"""
...
def make_compute_chi_split(Hdt_thr):
    def compute_chi_split(self, u, dt, threshold=None, width=None, transition="inv"):
        ...
        # Scalable Hessian penalty: H*dt instead of H > 800
        H_dt = H_norm * dt
        arg_h = (H_dt - Hdt_thr) / (Hdt_thr * 0.5)  # width = half the threshold
...
def run_case(case_name, Hdt_thr):
    AdvReactUni1DEval.compute_chi_split = make_compute_chi_split(Hdt_thr)
...
print(f"{'Hdt_thr':<10s} | {'A':<12s} {'B':<12s} {'C':<12s}")
print("-" * 50)

for Hdt_thr in [5.0, 8.0, 10.0, 12.0, 15.0, 20.0, 30.0]:
    errs = {}
    for case in ['A', 'B', 'C']:
        errs[case] = run_case(case, Hdt_thr)
    print(f"{Hdt_thr:<10.1f} | {errs['A']:.2e}   {errs['B']:.2e}   {errs['C']:.2e}")
    sys.stdout.flush()
