"""
Test explicitly dimensionless penalty: H * dt * u_ref
where u_ref is the model-dependent reference scale.
"""
...
def make_compute_chi_split(thr):
    def compute_chi_split(self, u, dt, threshold=None, width=None, transition="inv"):
        ...
        # Explicitly dimensionless Hessian penalty: H * dt * u_ref
        u_ref = self._get_u_ref(u)
        measure = H_norm * dt * u_ref
...
def run_case(case_name, thr):
    AdvReactUni1DEval.compute_chi_split = make_compute_chi_split(thr)
...
print("Explicitly dimensionless penalty: H * dt * u_ref")
print(f"{'thr':<8s} | {'A':<12s} {'B':<12s} {'C':<12s}")
print("-" * 45)
for thr in [8.0, 10.0, 12.0, 15.0, 20.0]:
    errs = {case: run_case(case, thr) for case in ['A', 'B', 'C']}
